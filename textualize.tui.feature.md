...
*   **Implementation:**
    ...
    2.  In `aider/tui.py`, define a minimal Textual app:
        ```python
        ...
        def run_tui(args):
            ...
            app.run()
        ```
    3.  In `aider/main.py`, modify the `if args.tui:` block to import and call the TUI runner. This import must be local to the `if` block to avoid impacting CLI startup time.
        ```python
        if args.tui:
            try:
